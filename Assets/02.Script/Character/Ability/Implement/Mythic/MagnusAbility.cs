    ...
    public override void CastAbility(CharacterBase characterBase)
    {
        ...
        foreach (Collider2D hit in hits)
        {
            if (hit.CompareTag("Enemy"))
            {
                ...
                enemyBase.TakeDamage(characterBase.GetApplyAttackDamage(characterBase.heroInfo.attackDamage) * 20 + ((!isMakdus || !isAlisda) ? enemyBase.maxHp * 0.04f : enemyBase.maxHp * 0.08f), characterBase.heroInfo.damageType);
            }
        }
    }
